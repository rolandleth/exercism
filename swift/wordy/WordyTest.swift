...
class WordyTest: XCTestCase {
	...
	func testRaisedPower() {
		let question = "What is 2 raised to the 3rd power?"
		XCTAssertEqual(8, try? WordProblem(question).answer())
	}
	
	func testRaisedPowerAndAddition() {
		let question = "What is 2 raised to the 3rd power plus 5?"
		XCTAssertEqual(13, try? WordProblem(question).answer())
	}
}
